export const DEFAULT_RERANKING_WEIGHTS = {
    vectorWeight: 0.35,
    bm25Weight: 0.25,
...
export function computeRecencyScore(createdAtIso, halfLifeDays = 30) {
    const ageMs = Date.now() - new Date(createdAtIso).getTime();
    const ageDays = Math.max(0, ageMs / (1000 * 60 * 60 * 24));
    // Exponential decay
    return Math.exp((-Math.LN2 * ageDays) / halfLifeDays);
}
export function computeTemporalValidityScore(memory, targetDateIso) {
...
export function computeAccessScore(accessCount) {
    return Math.min(1.0, Math.log1p(accessCount) / Math.log1p(50));
}
export function rerankMemories(candidates, options) {
    ...
    const scoredResults = candidates.map((cand) => {
        ...
        let finalScore = vScore * weights.vectorWeight +
        ...
        if (vScore > 0.7)
            reasons.push(`High semantic similarity (${(vScore * 100).toFixed(0)}%)`);
        if (bScore > 0.7)
            reasons.push(`Exact keyword match`);
        if (impScore >= 0.8)
            reasons.push(`High priority memory`);
