...
export function formatMemoryContextBlock(memories: Memory[]): string {
  ...
  const items = memories.map((mem, index) => {
    ...
    if (mem.valid_from) metaParts.push(`Valid From: ${mem.valid_from}`);
    if (mem.valid_to) metaParts.push(`Valid To: ${mem.valid_to}`);
    if (mem.source_provider) metaParts.push(`Source: ${mem.source_provider}`);
