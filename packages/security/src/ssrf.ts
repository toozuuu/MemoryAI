...
export async function validateUrlForSSRF(rawUrl: string, allowedHostnames?: string[]): Promise<string> {
  let parsed: URL;
  try {
  ...
  try {
    ...
    for (const record of addresses) {
      if (isPrivateIp(record.address)) {
        throw new SSRFError(
          `Destination hostname '${hostname}' resolved to prohibited IP '${record.address}'`
        );
      }
    }
  } catch (err: unknown) {
    if (err instanceof SSRFError) throw err;
    throw new SSRFError(`DNS resolution failed for hostname '${hostname}': ${(err as Error).message}`);
  }
